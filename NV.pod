...
=head1 FUNCTIONS
   ...
   $string = hex_alt($val, $fmt);
    $val must be either a mant_dig() precision Math::MPFR object or a perl
    ...
    Else the hex string returned is as formatted by the mpfr library.
    The formats provided by perl and mpfr (which are sometimes quite
    different) should always represent equivalent values.
